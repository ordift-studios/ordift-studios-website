...
- **External Workforce Portal** (`/portal/collaborator`, `/portal/vendor`, `/portal/model`) — a shared assignment/compensation/files/feedback surface for any relationship whose engagement the logged-in account owns.
...
**Known open items** (tracked in `TECHNICAL_DEBT_REGISTER.md`): TD-052 (activity-log sensitivity tiering — resolved same phase this entry was written), TD-053 (`recordManualPayment()` lacks a status-transition guard), TD-054 (`service_role` has no *explicit* grant on `currencies`/`bank_accounts`/`payment_country_config`), TD-055 (payable audit-metadata `fieldsChanged` cosmetic over-reporting).
