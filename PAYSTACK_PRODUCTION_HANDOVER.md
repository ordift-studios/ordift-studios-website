...
4. **[PRODUCTION]** Promote migrations 0023 → 0024 → 0025 → 0027, in that exact order (§2 of this document is now stale on the exact list — see `PRODUCTION_READINESS_RECONCILIATION.md` §2 for the current, correct sequence and dependency reasoning; `0026` is already live on Production). **Same day, immediately after:** migration 0024 creates Production's first-ever Supabase Storage bucket (`payment-proofs`) — `pg_dump` doesn't capture Storage objects, so a fresh manual database backup is required right after promotion (don't wait for the next weekly cycle), **and, per the Storage-backup decision now recorded in `DISASTER_RECOVERY.md` §4 (Option A until this promotion, Option B mandatory from this promotion onward), a `payment-proofs` Storage object export must be performed in the same operational sitting as that database backup** — one combined backup event, not two separately-scheduled ones. Exact combined procedure: `PRODUCTION_READINESS_RECONCILIATION.md`'s pre-migration execution brief.
