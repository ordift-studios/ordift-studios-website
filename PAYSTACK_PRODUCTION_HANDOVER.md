...
4. **[PRODUCTION]** Reconcile migration 0026's tracking gap, then promote 0023 → 0024 → 0025 → 0026 (§2). **Same day, before anything else:** migration 0024 creates Production's first-ever Supabase Storage bucket (`payment-proofs`) — `pg_dump` doesn't capture Storage objects, so `DISASTER_RECOVERY.md`'s existing backup routine has zero coverage for it starting the moment this promotion runs. Take a fresh manual backup immediately after promoting (don't wait for the next weekly cycle — everything since the 2026-07-30 backup, including all of this migration's new tables, is otherwise uncovered), and decide + document in `DISASTER_RECOVERY.md` §4 whether proof-of-payment files need their own backup step or whether the "re-request from the client if lost" risk is acceptable. Flagged during the 2026-08-10 Workstream H disaster-recovery re-review — see `DISASTER_RECOVERY.md`'s updated §4 for the full reasoning.
