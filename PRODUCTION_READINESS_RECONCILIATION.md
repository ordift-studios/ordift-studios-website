...
## 21. Site-wide link crawl + console-error sweep (task #283, 2026-08-13/14)

Continuation of the readiness/debt register's site-wide crawl (task #283), scoped to a comprehensive link/console/redirect sweep across public marketing pages, auth routes, Client/Collaborator Portal, and every Admin module, on staging only, per the same category-1/2/3/4 classification instruction used elsewhere in this engagement.

**Scope check first:** confirmed no prior pass had combined public+portal+admin link/console coverage in one sweep — the closest precedent (a 2026-07-30 recursive crawl) was public-only and already stale; #281/#282's "no console errors" notes were side-observations of interaction/permission-focused testing, not a dedicated link-crawl deliverable. Several admin pages (Enquiries, Content, Reports, Portfolio + subroutes, Payments, Exchange Rates) and the "workshop" kind of the Client Project Workspace had never been crawled in any prior pass.

**Test method:** three disposable accounts — one combined account (`client`+`workshop_participant`+`vendor`+`model`) to reach every Client/Vendor/Model/Workshops portal route in one login, one `contractor` account for the Collaborator Portal, and one `super_admin` account (chosen since #282 already exhaustively verified the staff/admin/super_admin authorization boundary itself — this pass only needed to reach every admin page's links/console, not re-verify who can reach it). Plus one `enquiries` row, one `workshop_registrations` row, and one `project_assignments` row linking the contractor to the enquiry. Same `.invalid`-domain fixture mechanism, fail-closed staging-host guard, used throughout this engagement.

**Coverage achieved:** all 39 `sitemap.xml` URLs plus 5 non-sitemap public pages, robots.txt, and deliberate bad-slug 404s (bulk-fetched, cross-referenced against `read_network_requests` for status codes); auth/account routes and both Admin and Client-Portal unauthenticated-redirect boundaries; the full Client Project Workspace for both the `enquiry` and `workshop` kinds; the Collaborator assignment-detail flow; and every Admin module (Overview, Enquiries list + detail, Content, Reports, Portfolio dashboard + Categories + Collections + New Project + a real project's detail/edit, Payments, Exchange Rates, Bookings list + detail, Activity, Flags, Lookups, Users, Settings, own Profile). No areas were inaccessible or left untested.

**Findings:** zero broken links, zero unexpected redirects/404s, zero new console errors anywhere in the site. One Category-1 application defect found: the Admin Enquiries list (`src/app/admin/enquiries/page.tsx`) and detail (`src/app/admin/enquiries/[id]/page.tsx`) pages rendered the `service` field through an ad hoc `.replace(/-/g, " ")` + Tailwind `capitalize` transform instead of the existing `pathwayLabel()` helper (`src/lib/enquiry/pathways.ts`) that every other consumer of this field already uses. For the fixed pathway enum this coincidentally looked correct, but it silently corrupted any hyphenated free-text value outside the enum (surfaced by this pass's own test fixture, whose service value read "Site Wide Crawl Validation" instead of the stored "Site-wide crawl validation" — a display-only bug, the underlying data was never touched).

**Fix (Category 1 — display-only, approved and applied):** both pages now call `pathwayLabel(enquiry.service)` / `pathwayLabel(e.service)` in place of the inline transform, matching the pattern already used by the booking/portal/email code paths. Scoped strictly to these two files — no data, schema, RLS, authorization, or payment logic touched.

**Regression-tested live on staging using the existing fixture before cleanup:** the fixture's hyphenated free-text value now renders unmodified on both pages; the real, pre-existing `Photography` pathway-enum enquiry (unrelated to this fixture) still renders its correct label; the underlying stored `service` value was independently confirmed unchanged via a read-only database check; zero new console errors on either page.

**Cleanup:** the fixture's deliverables/project_requests/project_updates (0 found — none were created during this link-focused pass), `activity_log` rows for the three fixture accounts (0 found), the `project_assignments` row, the `enquiries` row, the `workshop_registrations` row, and all three disposable auth accounts (profile/role-grant cascaded) were deleted. The cleanup script's own final read-only check confirmed **0 fixture rows/accounts remaining** before it removed its own record file. The two throwaway scripts (`_createSiteWideCrawlFixtures.ts`, `_cleanupSiteWideCrawlFixtures.ts`) were then deleted — never committed, no longer needed.

**Verification:** `npx tsc --noEmit` clean. `npx eslint src/app/admin/enquiries/page.tsx "src/app/admin/enquiries/[id]/page.tsx"` clean. Full `vitest run`: 65/65 passing, plus the relevant `bookingWorkflow.integration.test.ts` (6/6) — no regressions.

No schema, Paystack, CSP, Sentry, Supabase Production, Cloudflare, DNS, or Vercel Production changes. No Production data touched — staging only. Not merged to `main`. `src/app/admin/enquiries/page.tsx`, `src/app/admin/enquiries/[id]/page.tsx`, and this document are the only files changed in this pass.

**Task #283 (Site-wide link crawl + console-error sweep) is now complete.** Next up per the existing register: task #284 (API endpoint sanity checks) — not started, pending separate instruction.

## 22. API endpoint sanity checks (task #284, 2026-08-13/14)

Continuation of the readiness/debt register's API sanity pass (task #284), scoped to method/auth/validation/error-handling correctness across every Route Handler under `src/app/api/`, on staging only, per the same category-1/2/3/4 classification instruction used elsewhere in this engagement.

**Scope check first:** cross-referenced the 12 existing route handlers against a 2026-08-02 API sanity pass logged in `MILESTONES.md` (validation/auth/404 baseline for `/api/enquiry`, `/api/workshop-registration`, `/api/admin/google-sheets/*`, `/api/admin/reports/*`, `/api/admin/resend/verify-send`) and the exhaustive live-HTTP payment testing documented in `PAYMENT_FINANCE_ARCHITECTURE_PROPOSAL.md` §16/§20 (real signed webhook deliveries, 10-way concurrency/idempotency test, real authenticated bank-transfer proof/approve/reject sessions). Two routes postdate that coverage entirely — `/api/admin/portfolio/assets` (native Portfolio editor, built after 2026-08-02) and the admin report export/email routes' auth/edge-case behavior specifically (previously only exercised via UI clicks, never as raw endpoints) — and were this pass's primary focus, alongside a full method/malformed-body/authorization sweep of every route.

**Test method:** one disposable client-tier account + one disposable `enquiries` row (amount_due set to mirror the real "Set Amount Due" admin action) for bank-transfer write-path testing; one disposable staff-tier account, added mid-pass with your approval, to reach the report endpoints' staff-gated success path. Same `.invalid`-domain fixture mechanism, fail-closed staging-host guard, used throughout this engagement. `/api/admin/resend/verify-send` and `/api/admin/google-sheets/verify-write` were never called (already proven live by #401 and Phase 2A respectively; both touch a real external system); `/api/admin/google-sheets/setup` was called **GET only**, never POST.

**Coverage achieved:** all 12 route handlers exercised for unsupported-HTTP-method behavior (clean 405s), malformed/missing-JSON-body handling, unauthenticated access (401/403, fail-closed, no pre-auth existence leak on unknown resource keys), insufficient-role access (client-tier session against every staff/admin/super-admin route), ownership boundaries (bank-transfer proof/PUT against unowned/nonexistent entities and payments), status-guard boundaries (resubmission after `awaiting_verification` → 409), a full real write-path (`PUT` → `POST` proof upload → independently verified via read-only DB/Storage query), rate-limiting reconfirmation (5-per-10-min window on bank-transfer PUT correctly triggered 429), and — after the staff fixture was added — the full success path of the report export/email endpoints (#6–#9), confirmed staging-safe (`sendReportEmail()` always logs rather than sends when `productionSendingEnabled()` is false, which staging always is).

**Findings:** one Category-1 application defect found and fixed. The Paystack webhook route (`src/app/api/payments/webhook/paystack/route.ts`) called `paystackProvider.verifyWebhookSignature()` with no try/catch; if `PAYSTACK_SECRET_KEY` is unset or misconfigured in the runtime, the underlying `requireSecretKey()` throw propagated as an unhandled exception, producing a generic 500 instead of the route's own documented fail-closed 401. Verified via `curl -i` that the 500 response leaked nothing (empty body, standard security headers only) — a robustness gap, not an information-disclosure or authorization issue. Reproduced only because the **local** dev environment lacks `PAYSTACK_SECRET_KEY` (deployed staging has it, per task #430); this is a genuine source-level gap regardless of which environment currently has the key configured.

**Fix (Category 1 — approved and applied):** wrapped the `verifyWebhookSignature()` call in try/catch, returning the same 401 `{ ok: false }` response on any thrown error, matching the route's own stated "fail closed on any failure" design intent. Scoped to this one function call — no other webhook logic (replay window, idempotency, amount validation, entity sync) touched.

**Re-tested after the fix, all confirmed:** missing signature header → 401; garbage signature header under a missing secret key (previously 500) → now 401, `{"ok":false}`, no leaked detail; `npx tsc --noEmit` and `npx eslint` on the changed file both clean. Valid-signature behavior is provably unchanged by code review (the try/catch wraps only the existing call, with no change to any downstream logic) — the success path was already extensively live-proven against the deployed environment in §16/§20 and was not re-run here, since re-verifying it doesn't need a local secret and nothing about it changed.

**No other findings.** Every other route/case matched source behavior exactly: correct status codes throughout (400/401/403/404/409/413/415/422/429 as appropriate), no stack traces or internal details in any error body, auth checks consistently run before any parsing/business logic on every protected route, and the bank-transfer write sequence produced exactly the expected database/Storage state with no anomaly.

**External-action confirmation:** zero real Paystack API calls (the webhook route only receives; all signature/malformed-body tests were either unsigned or run against a local environment lacking the secret key, so no genuine payload was ever processed); zero real emails sent (`resend/verify-send` never called; report-email routes confirmed staging-safe by source and by their `mode: "logged"` response); zero Google Sheets writes beyond the one read-only `setup` GET.

**Cleanup:** the fixture's `payments` row, its Storage proof object, the `enquiries` row, `activity_log` rows for both fixture accounts (0 found for either), and both disposable auth accounts (client + staff, profile/role-grant cascaded) were deleted. The cleanup script's own final read-only check confirmed **0 fixture rows/accounts/objects remaining** before it removed its own record file. All seven throwaway scripts used during this pass were then deleted — never committed, no longer needed.

**Verification:** `npx tsc --noEmit` clean. `npx eslint src/app/api/payments/webhook/paystack/route.ts` clean. Full `vitest run`: 65/65 passing, no regressions. No dedicated automated test exists for this route (payments testing in this codebase has consistently been real-HTTP/live-verified rather than unit-tested, per §16/§20) — this pass's live re-test is the equivalent verification.

No schema, Paystack, CSP, Sentry, Supabase Production, Cloudflare, DNS, or Vercel Production changes. No real Paystack, email, or Google Sheets external action occurred. No Production data touched — staging only. Not merged to `main`. `src/app/api/payments/webhook/paystack/route.ts` and this document are the only files changed in this pass.
...
## 23. Task #285 — Fix all safely-fixable engineering issues found (2026-08-14)

**Scope:** an exhaustive inventory of every unresolved engineering issue from prior readiness passes (`TECHNICAL_DEBT_REGISTER.md`, all 35 entries, plus §6/§7 above), cross-referenced against current source and staging `HEAD`, classified as: safe to fix now / already resolved-obsolete / intentional design / deferred tech debt / requires business decision / requires external credentials / Production-infra-must-not-touch. Only **TD-030** (file uploads validate declared MIME type, not actual content) and **TD-033** (bank-transfer proof submission missing `activity_log` entries) were approved as safe to fix now, in that order — every other open item was either already resolved, intentional design, blocked on a business/product decision, or explicitly Production/infrastructure work out of scope.

**TD-033 (implemented first, per approval order):** see `TECHNICAL_DEBT_REGISTER.md`'s TD-033 entry for the full architectural-gap writeup and verification detail. In short: implementing the literal pay-down trigger (`logActivity()`) would have silently created zero audit rows for an ordinary client's own submission, since `activity_log`'s only insert RLS policy is staff-only. Flagged before proceeding; user approved a narrowly-scoped `logActivityAsSystem()` exception under explicit security boundaries (server-verified `actorUserId` only, reachable only after auth/ownership/validation succeed, best-effort/log-and-continue, no new RLS policy or migration). Two new actions added: `payment.bank_transfer_initiated` (`PUT`), `payment.bank_transfer_submitted` (`POST`).

**TD-030 (implemented second):** a pre-implementation compatibility investigation (format coverage, Node version, ESM/bundler compatibility, buffering impact) found no blockers — reported to the user before installing anything, per the approval's explicit stop conditions. `file-type@22.0.1` installed; new `detectFileMimeType()` (`src/lib/shared/fileContentSniff.ts`) wired into both `src/app/api/payments/bank-transfer/proof/route.ts` and `src/app/api/admin/portfolio/assets/route.ts` as a magic-byte check that supplements, never replaces, each route's existing declared-`Content-Type` allow-list.

**Verification (both fixes, via disposable staging fixtures — one client account + enquiry, one `super_admin` account):**
- Bank-transfer proof route: full `PUT` → `POST` success path re-run for jpeg/png/webp/pdf (all 4 currently-allowed formats) — each left the payment `awaiting_verification` with the correct `proof_of_payment_asset_path`, a matching Storage object, and exactly the expected `activity_log` entries (2 per payment, correctly actor-attributed). 7 rejection-path cases (spoofed sniffed-type, renamed text file, empty file, malformed bytes, oversized, disallowed declared type, nonexistent `paymentId`) all correctly rejected with **zero** activity_log rows created — confirming TD-033's "no misleading successful-action entries" requirement holds under TD-030's own negative test matrix.
- Portfolio assets route: all 6 currently-allowed formats (jpeg/png/gif/webp images; pdf/zip files) succeeded end-to-end through Sanity's real asset pipeline, each producing exactly one `portfolio.asset_uploaded` activity_log row. 7 rejection-path cases (spoofed sniffed-type, renamed text files ×2, empty file, malformed bytes, oversized, PDF content submitted under `kind=image`) all correctly `415`/`413`. Unauthenticated request correctly `403`'d, confirming the existing `canCreatePortfolioProjectsNatively` gate is unaffected by either change.
- Actor-identity regression check (TD-033-specific): a staff-role fixture submitting proof on a client's behalf correctly attributed the *staff* actor to `payment.bank_transfer_submitted`, while the client's own earlier `payment.bank_transfer_initiated` entry still correctly showed the *client* — `actorUserId` reflects the true human actor per call, never a stale or assumed identity.

**Full closeout checklist:** `npx tsc --noEmit` clean; `npx eslint` on all 4 changed source files clean; full `npx vitest run` — 65/65 passing, no regressions; `package.json`/`package-lock.json` diff re-inspected — exactly one new declared dependency (`file-type@^22.0.1`) and 7 new transitive entries, zero modifications to any existing package version; all disposable fixtures (2 auth accounts, 1 enquiry, 4 payments rows, 4 Storage objects, 6 Sanity assets, 14 activity_log rows) independently re-verified as fully removed via a final read-only pass before their own cleanup scripts were deleted; all throwaway scripts and the fixture-record JSON deleted, never committed.

**No schema, Paystack, CSP, Sentry, Supabase Production, Cloudflare, DNS, or Vercel Production changes.** No real external-service action occurred (Sanity staging dataset asset writes were disposable-test-only and fully deleted). Not merged to `main`. Permanent files changed: `src/lib/admin/activityLog.ts`, `src/app/api/payments/bank-transfer/proof/route.ts`, `src/app/api/admin/portfolio/assets/route.ts`, `src/lib/shared/fileContentSniff.ts` (new), `package.json`, `package-lock.json`, `TECHNICAL_DEBT_REGISTER.md`, and this document.

**Task #285 is now complete**, pending the user's review of the diff and explicit approval before commit/push.
