...
**Incident (2026-07-28) — invited Staff account couldn't log in.** A real invited Staff member (`mishaeladjei54@gmail.com`, correctly Active with the Staff role, an Operational Title, and Full-time engagement — everything visible in Staff Management looked right) still couldn't sign in. Full trace: middleware doesn't gate by role at all; `/admin` explicitly allows Staff (`isStaffOrAdmin()`); there is no separate "Staff Portal" to be missing — Staff and Admin share `/admin` by design (Task #85). The account's `auth.users`/`profiles`/`staff_details`/`user_roles` rows were all correctly linked. **Root cause:** `inviteCollaboratorAction` redirected the invite link to `/portal/login`, a plain sign-in form with no code to read the invite's session token from the URL. The Supabase client SDK silently consumed the token on page load (confirming the email, logging one `last_sign_in_at`), but nothing ever prompted the person to set a password — so the account was left permanently password-less, and every later login attempt correctly failed with the generic "Invalid email or password." This looked identical to an authorization bug from the outside but was purely an authentication gap: the account, its role, and every guard around `/admin` were correct the whole time. **Fix:** redirect changed to `/portal/reset-password` (`src/app/admin/users/actions.ts`), which already parses this exact hash-token format (`ResetPasswordForm.tsx` — built for the forgot-password flow, and it doesn't care whether the token's `type` is `recovery` or `invite`) and walks the person through setting a password before landing them back on the login page. Verified by inviting a fresh throwaway Staff account end-to-end — accept invite → set password → sign in → land on `/admin` — with zero manual intervention, then deleting that test account. Mishael was unblocked directly via the existing "Forgot password?" flow (no code change needed for an already-invited account — a resend of the invite isn't necessary once a password can be set through recovery instead).
...
Clicking your own name in the `/admin` header (`src/app/admin/layout.tsx`) opens a Quick Card (`src/components/admin/ProfileQuickCard.tsx`) with Member Number, Classification, Job Title, Department, Grade, Date Joined, calculated tenure, Account Status, and Last Login, plus "View Full Profile" and "Edit Profile" links to `/admin/profile/[id]` (`src/app/admin/profile/[id]/page.tsx`).
...
**Editing — split by risk, not just by ownership.** Contact fields (Full Name, Phone) are self-service: any admin can edit their own via the existing `profiles` self-update grant (`src/app/admin/profile/[id]/actions.ts` → `updateOwnContactDetailsAction`, request-scoped client, RLS-enforced). Operational fields (Job Title, Department, Grade) and Account Classification are **Super Admin-only to edit, even on your own card** — these are admin-managed facts, not self-service — via `updateStaffOperationalDetailsAction`/`reclassifyAccountAction`, both gated by `isSuperAdmin()` and using the service-role client, the same precedent as `updateCollaboratorDetailsAction` in `src/app/admin/users/actions.ts`.

**Member Number** superseded the old staff-only Staff Number on 2026-07-28 — see §18.

**Last Login** appears in two places: on a person's own Quick Card/Full Profile (any role, self-view only), and — added 2026-07-28 — as an "Account Info" line inside the "Manage" panel on `/admin/users` (Users & Roles), so Admin/Super Admin can see any account's last login while managing the full user list. That page already gates its entire contents to `hasRole(user, "admin")`; no separate check was added for this field.
...
## 18. Member Number System

Every account on the platform — staff or public-facing — has a **Member Number**: an auto-generated, prefixed identifier that lets you tell someone's role at a glance without opening their profile. It replaced the staff-only Staff Number (§17, pre-2026-07-28) on 2026-07-28, and is driven entirely by **Account Classification**, an explicit admin-assigned field — never derived or guessed from Role, Engagement Type, or Operational Title.

**Classifications, out of the box:** Permanent Staff (no prefix, e.g. `0001`), Intern (`IN0001`), Contractor (`CT0001`), Freelancer (`FL0001`), Volunteer (`VL0001`), Project Participant (`PR0001`), Instructor (`INT0001`), Workshop Participant (`WK0001`), Model (`MD0001`), Vendor (`VN0001`), Client (`CL0001`). Each has its own independent, never-resetting counter — reclassifying doesn't affect anyone else's numbers, and no counter goes backward even if a classification sits unused for a while.

**Fully configurable — no code required.** Settings → Titles & Classifications (`/admin/lookups`, Super Admin only) lets you create new classifications, set their prefix/digit-padding/starting number, rename them, or disable one, via `src/app/admin/lookups/ClassificationManager.tsx` and `classificationActions.ts`. A disabled classification can't be newly assigned but existing holders keep their number.

**Assignment is always automatic, never manual entry.** Three paths: (1) public self-signup auto-assigns "Client"; (2) admin-invited collaborators (`/admin/users` → Invite) require picking a classification at invite time; (3) any Super Admin can reclassify an existing account from their Full Profile edit view (`/admin/profile/[id]`) or from `/admin/users`. There is no field anywhere to type a Member Number directly.

**Lifecycle — archive, never reuse.** The Member Number changes *only* when Account Classification changes — editing Job Title, Department, Grade, or permissions never touches it. Reclassifying archives the current active row in `public.member_numbers` (`status='archived'`, `archived_at` timestamped) and inserts a new active row; the old number stays in the table permanently for history/audit, never deleted, and is never reassigned to anyone else — even if the same person is later reclassified back to a classification they previously held, they get a *new* number in that classification's sequence, not the old one back. Reclassifying to the classification already held is a no-op (idempotent) — no wasted number, no duplicate activity-log entry.

**Schema:** migration `0019` — `public.member_number_classifications` (the configurable lookup) and `public.member_numbers` (append-only ledger, RLS: read own or staff/admin, no `authenticated` write policy at all — every write goes through the service-role application code in `src/lib/portal/memberNumbers.ts`, never a direct client write). `profiles.member_number` is a denormalized cache of the current active number, kept in sync transactionally, so every read site can display it with a plain query instead of a join.

## 19. Admin Presence Panel ("Active Now")

`/admin/overview` shows a live **"Active Now"** panel, positioned above Recent Activity: Name, Member Number, Department for every Staff/Admin/Super Admin account currently viewing any `/admin/**` page, updating in real time as people log in, log out, or go idle — no page refresh needed.

**How it works:** Supabase Realtime Presence on a channel named `admin-presence`, mounted once in `src/app/admin/layout.tsx` (`PresenceProvider`, `src/components/admin/PresenceProvider.tsx`) so a person shows as present anywhere in the admin console, not only while literally on the Overview page. A 5-minute client-side idle timer (mouse/keyboard/scroll/click resets it) marks someone "Away" without dropping their connection — Presence alone can only tell you a tab is open, not whether anyone's using it.

**Security — private channel, not just route-gating.** Realtime channels aren't private by default: any authenticated Supabase client can join any channel by topic name from browser dev tools, regardless of what the app's UI renders. The channel is created with `{ config: { private: true } }`, and Postgres RLS policies on `realtime.messages` (migration `0020`) restrict both subscribing and broadcasting to Staff/Admin/Super Admin accounts specifically — a Client, Model, Vendor, or Workshop Participant account is rejected at the database level even if they open the channel manually, the same trust boundary as every other internal-only surface in this schema, not a separate one.

**Known issue, fixed:** the standard `/admin` → `/admin/overview` server redirect (hit on every login) briefly double-mounts the layout, which can leave two concurrent presence connections alive under one person's key for a moment. Dedup logic is recency-based — each tracked update carries a timestamp, and the most recent one always wins — rather than a naive "prefer online," which could otherwise get stuck showing someone as permanently online if a stale connection's last write happened to be "online."
