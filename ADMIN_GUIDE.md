...
**Staff Numbers** are a plain zero-padded sequence with no visible prefix or year (e.g. `000001`) — not part of the `PREFIX-YYYY-NNNNNN` family (`RECORD_ID_STANDARD.md`), deliberately, so the value doesn't announce "this is a staff record" to a casual viewer, and doesn't duplicate the year already shown in Date Joined. Reuses the existing `public.next_record_sequence()` function (migration `0013`) via `generateStaffNumber()` (`src/lib/shared/recordId.ts`), keyed by an internal, never-displayed `"STAFF"` prefix with a fixed year of `0` (not a real calendar year) so the counter never resets annually — since the year isn't shown, a per-year counter would eventually produce two different people with the same visible number. Nullable, assigned lazily via the "Assign Staff Number" button on a Super Admin's edit view; never auto-backfilled for existing accounts.
