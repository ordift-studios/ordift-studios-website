...
1. **Legal pages — review and approve** (highest-priority remaining item). Real, tailored draft text — grounded in what the site actually collects and does, not generic boilerplate — is now loaded into all four (`privacy`, `terms`, `cookies`, `booking`) on both datasets, with `isApproved` still `false`. Two sections in Booking Terms (payment/deposit terms, cancellation policy) are explicitly marked `[Business note: ...]` since I deliberately did not invent numbers you haven't set. Read the drafts (in Sanity Studio, or ask me to paste them here), confirm or amend, then tell me to flip `isApproved: true` — a fast, low-risk change once you're satisfied.
