...
**Status:** live in Production. Rollback reference, if ever needed: `35ac583` (the pre-promotion Production baseline), via revert-and-push — no data migration exists in this promotion to unwind. Pulse's operational state is unchanged by this promotion — discovery/auto-publish remain off, `pulseSource.researchPetapixel` remains inactive pending a separate licensing review; this was an integrity/architecture fix, not a decision to resume active Pulse content operations. *(Historical note, preserved rather than rewritten: this was the accurate, intended state at the time of this 2026-09-03 promotion — a deliberate architecture/integrity fix, explicitly not a decision to activate Pulse.)*

**Superseded (2026-09-10) — the "remains inactive" line above no longer describes the current state and must not be read as current.** A subsequent investigation (`TECHNICAL_DEBT_REGISTER.md` TD-067, Resolved) established that PetaPixel's `isActive` flag and the global `pulseSettings.discoveryEnabled` switch were both already set to `true` on **2026-08-25** — during legitimate Phase B testing, predating this 2026-09-03 entry by over a week — and were never reverted; no record of an explicit decision to keep them on (or to turn them off again) exists. Once a real Vercel Cron for `/api/cron/pulse-discovery` was added 2026-09-08, it began firing against this already-active source automatically the very next day. **Current authoritative state, confirmed via Founder authenticated Production verification (`/admin/pulse`, 2026-09-10): PetaPixel is active, the cron executes automatically and has been independently verified PASS, and real drafts exist in Sanity's private `drafts.*` namespace — never published without human review.** See TD-067 for the full evidentiary chain. A future reader should treat *this* paragraph, not the 2026-09-03 line above it, as the current fact.
