...
describe("six-jurisdiction capability taxonomy — no cross-jurisdiction overlap", () => {
  ...
  it("the finance.* compensation/payout/payment-obligation capabilities match the exact strings specified (Phase 3.4), plus Workshop Management's workshopRevenueView (Phase B), the Universal Payables System's payeeAdminister/paymentObligationRecordPayment (2026-09-03), Payable Safety Hardening's paymentObligationReverse (2026-09-04), and Pricing Engine V1's pricingAdminister (2026-09-06)", () => {
    expect(FINANCE_CAPABILITIES).toEqual({
      ...
      pricingAdminister: "finance.pricing.administer",
    });
  });
});
