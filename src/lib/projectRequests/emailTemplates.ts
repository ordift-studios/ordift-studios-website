import type { ProjectRequestRecord } from "./sheetsSync";

// Same email-safe font stacks and palette as the enquiry/workshop
// emails (src/lib/enquiry/emailTemplates.ts,
// src/lib/workshops/registrationEmailTemplates.ts) — kept as a local
// copy rather than a shared import, matching the same "structurally
// independent system" convention those two already follow.
const SERIF = "Georgia, 'Times New Roman', serif";
const SANS = "-apple-system, Segoe UI, Roboto, Arial, sans-serif";
const NAVY = "#0B1220";
const GOLD = "#BFA14A";
const OFFWHITE = "#F7F5F1";
const INK_MUTED = "#5B5F6B";

function wrap(bodyHtml: string): string {
  return `<!DOCTYPE html>
<html>
  <body style="margin:0;padding:0;background:${OFFWHITE};font-family:${SANS};">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${OFFWHITE};padding:32px 16px;">
      <tr>
        <td align="center">
          <table role="presentation" width="100%" style="max-width:560px;background:#ffffff;border-radius:12px;overflow:hidden;">
            <tr>
              <td style="background:${NAVY};padding:28px 32px;">
                <span style="font-family:${SERIF};font-size:20px;color:#ffffff;letter-spacing:0.02em;">ORDIFT STUDIOS</span>
              </td>
            </tr>
            <tr><td style="height:3px;background:${GOLD};"></td></tr>
            <tr>
              <td style="padding:32px;">
                ${bodyHtml}
              </td>
            </tr>
            <tr>
              <td style="padding:20px 32px;border-top:1px solid #eeeeee;">
                <p style="margin:0;font-family:${SANS};font-size:12px;color:${INK_MUTED};">
                  Ordift Studios · This is an automated message regarding your project request.
                </p>
              </td>
            </tr>
          </table>
        </td>
      </tr>
    </table>
  </body>
</html>`;
}

export function buildProjectRequestAcknowledgementEmail(record: ProjectRequestRecord) {
  const reference = record.referenceNumber ?? "pending";
  const subject = `We've received your request — ${reference}`;

  const bodyHtml = `
    <h1 style="margin:0 0 16px;font-family:${SERIF};font-size:24px;color:${NAVY};font-weight:normal;">
      Thank you, ${escapeHtml(record.clientName.split(" ")[0] || record.clientName)}.
    </h1>
    <p style="margin:0 0 16px;font-family:${SANS};font-size:15px;line-height:1.6;color:${NAVY};">
      We've received your <strong>${escapeHtml(record.requestTypeLabel)}</strong> request for
      ${escapeHtml(record.relatedProjectType)} ${escapeHtml(record.relatedProjectReference)}. Your reference number is:
    </p>
    <p style="margin:0 0 20px;font-family:${SANS};font-size:18px;font-weight:bold;color:${NAVY};letter-spacing:0.03em;">
      ${escapeHtml(reference)}
    </p>
    <p style="margin:0 0 20px;font-family:${SANS};font-size:14px;line-height:1.6;color:${INK_MUTED};">
      Submitting this request does not confirm it has been actioned. Our
      team will review it and follow up with a decision or any questions.
    </p>
    <p style="margin:0;font-family:${SANS};font-size:14px;line-height:1.6;color:${INK_MUTED};">
      You can track the status of this request from your project workspace
      at any time.
    </p>
  `;

  return { subject, html: wrap(bodyHtml), text: toPlainText(record) };
}

export function buildProjectRequestAdminNotificationEmail(record: ProjectRequestRecord) {
  const reference = record.referenceNumber ?? "pending";
  const subject = `New project request — ${record.requestTypeLabel} — ${reference}`;

  const rows: [string, string][] = [
    ["Reference", reference],
    ["Request type", record.requestTypeLabel],
    ["Related project", `${record.relatedProjectType} — ${record.relatedProjectReference}`],
    ["Submitted", record.createdAt],
    ["Client", record.clientName],
    ["Email", record.email],
    ["Phone / WhatsApp", record.phone || "—"],
  ];

  const rowsHtml = rows
    .map(
      ([label, value]) => `
      <tr>
        <td style="padding:6px 0;font-family:${SANS};font-size:13px;color:${INK_MUTED};width:160px;vertical-align:top;">${label}</td>
        <td style="padding:6px 0;font-family:${SANS};font-size:13px;color:${NAVY};">${escapeHtml(value)}</td>
      </tr>`
    )
    .join("");

  const bodyHtml = `
    <h1 style="margin:0 0 16px;font-family:${SERIF};font-size:20px;color:${NAVY};font-weight:normal;">
      New project request received
    </h1>
    <table role="presentation" width="100%" style="margin-bottom:20px;">${rowsHtml}</table>
    ${
      record.clientNotes
        ? `<p style="margin:0 0 4px;font-family:${SANS};font-size:13px;color:${INK_MUTED};">Client notes</p>
           <p style="margin:0;font-family:${SANS};font-size:14px;line-height:1.6;color:${NAVY};white-space:pre-wrap;">${escapeHtml(record.clientNotes)}</p>`
        : ""
    }
  `;

  return { subject, html: wrap(bodyHtml), text: toPlainText(record) };
}

function toPlainText(record: ProjectRequestRecord): string {
  return [
    `Reference: ${record.referenceNumber ?? "pending"}`,
    `Request type: ${record.requestTypeLabel}`,
    `Related project: ${record.relatedProjectType} — ${record.relatedProjectReference}`,
    `Client: ${record.clientName}`,
    `Email: ${record.email}`,
    record.clientNotes ? `Notes: ${record.clientNotes}` : "",
  ]
    .filter(Boolean)
    .join("\n");
}

function escapeHtml(value: string): string {
  return value
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;");
}
