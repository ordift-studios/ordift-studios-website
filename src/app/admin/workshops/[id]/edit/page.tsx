...
import { getWorkshopByIdAdmin, getWorkshopInternalNotesAdmin, getVenuesAdmin } from "@/lib/content/sanity/workshopAdmin";
...
export default async function EditWorkshopPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [workshop, venues, internalNotes] = await Promise.all([
    getWorkshopByIdAdmin(id),
    getVenuesAdmin(),
    getWorkshopInternalNotesAdmin(id),
  ]);
