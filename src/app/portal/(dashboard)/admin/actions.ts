...
export async function grantRoleAction(formData: FormData): Promise<void> {
  ...
  if (error) {
    console.error("[portal admin] grant role failed", error.message);
  }
...
export async function revokeRoleAction(formData: FormData): Promise<void> {
  ...
  // that happens (the Admin portal itself requires the admin role).
  if (roleSlug === "admin" && userId === currentUser.id) {
    console.warn("[portal admin] refused self-revoke of admin role", currentUser.id);
    return;
  }
  ...
  if (error) {
    console.error("[portal admin] revoke role failed", error.message);
  }
