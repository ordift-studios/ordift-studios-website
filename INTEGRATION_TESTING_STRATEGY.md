...
**Hard rule, no exceptions: integration tests never touch production.** Every credential, dataset, spreadsheet, and API key used by the integration-test suite is the existing **staging** equivalent — the same staging Supabase project, staging Sanity dataset pattern (ADR-005), and staging Resend/Sheets configuration already used throughout this project's many manual E2E verification passes (Milestones #55–#65, #182–#193, and others).
...
- **Google Sheets:** real calls against a dedicated test worksheet, real assertions on what was written — Sheets sync is a real dependency (ADR-003) and deserves a real test, just isolated to its own tab and mindful of quota (§2).
...
*Cross-references: `TECHNICAL_DEBT_REGISTER.md`, `ARCHITECTURE_DECISIONS.md` (ADR-002, ADR-003, ADR-005), `TECHNOLOGY_COST_REGISTER.md`, `STAGING.md`, `DISASTER_RECOVERY.md`, `ENGINEERING_GUIDE.md`, `PRODUCT_ROADMAP.md` (Version 1.0.5).*
