...
- **Member Number system + Admin Presence panel** — ✅ **Built and verified end-to-end on staging and production (2026-07-28).** Universal, admin-configurable Member Number (11 initial classifications, independently sequenced, archive-never-reuse lifecycle) replaces the staff-only Staff Number; a live "Active Now" panel on `/admin/overview` shows who's currently online via a Realtime-Authorization-gated presence channel restricted to Staff/Admin/Super Admin. Migrations `0019`/`0020` applied to both environments — see `ADMIN_GUIDE.md` §18–19 and `MILESTONES.md` for full detail, including two bugs found and fixed during verification (a presence-dedup edge case, and a `service_role` grant gap on `activity_log` — the fourth instance of the "Automatically expose new tables disabled on production" gap class, following `0010`/`0016`/`0018`; worth a dedicated audit pass across every table rather than fixing them one at a time as they're discovered).
