...
- **Google Sheets integration** — 🟡 **PENDING OWNER DECISION.** Rebuilt 2026-07-27 into a reusable, config-driven service covering 9 worksheets (2 live, 7 reserved) and inverted so Supabase is now primary/required and Sheets is a best-effort secondary with a `sheet_sync_failures` retry queue (see `GOOGLE_SHEETS_INTEGRATION.md`). Also needs migration `0013_record_ids_and_sheet_sync.sql` applied to staging and production before the new `PREFIX-YYYY-NNNNNN` record IDs can be generated (see `RECORD_ID_STANDARD.md`) — until then, both forms fail closed with a 503 rather than silently dropping a submission. Code complete; still needs a Google Cloud service account from you (`GOOGLE_SERVICE_ACCOUNT_EMAIL`, `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY`, `GOOGLE_SHEETS_SPREADSHEET_ID`).
