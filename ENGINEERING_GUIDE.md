...
**How to apply it going forward:** any new integration this codebase adds — a new third-party API, a new data store, a new notification channel — should default to reading its target/credentials from environment variables from the first line of code, not "for now hardcode it and parameterize later." Retrofitting this after the fact (as ADR-005 did have to do carefully for datasets) is more work than building it in from the start.
...
*Cross-references: `ARCHITECTURE_DECISIONS.md`, `TECHNICAL_DEBT_REGISTER.md`, `INTEGRATION_TESTING_STRATEGY.md`, `PRODUCT_ROADMAP.md` (Version 1.0.5, Workstream D), `STAGING.md`, `DOCUMENTATION_INDEX.md`.*
