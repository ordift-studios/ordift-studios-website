...
| `GOOGLE_SHEETS_MAPPING.md` | Documents the column layout the Google Sheets integration will write to, once connected (currently: **not yet connected**, see `PRODUCTION_READINESS_REPORT.md` §2) | When Google Sheets integration is actually wired up, or the mapping changes |
...
| `PULSE_ARCHITECTURE.md` | Ordift Pulse (Creative Industry Hub) — the taxonomy design (category/region/opportunity type as independent axes), the editorial/curated content model, the no-scraping trusted-source data layer, the editorial-approval workflow, and future-proofing for newsletters/personalization/saved articles/notifications/AI summaries. Architecture only — no public pages, no data provider integration yet | If the Pulse schema, workflow, or taxonomy changes, or when public pages/data-provider integration begin |
| `MEDIA_UPLOAD_LIST.md` | The shot list of every blank/placeholder media area on the public site — page, section, required type/orientation/dimensions/format, suggested subject, and whether it's ready for upload today or blocked on a schema change | When new media areas are built, or existing placeholders get wired up to render real content |
...
| `ADMIN_GUIDE.md` | The operational manual — roles/permissions, inviting and managing users, account lifecycle, email infrastructure, troubleshooting, and (as of 2026-07-27) Internal Governance standards | Whenever how the platform is operated day-to-day changes |
