...
| `DISASTER_RECOVERY.md` | The documented recovery procedure — current backup/PITR capability (audited live against the Supabase dashboard), an interim manual-backup method, database/Storage/env-var/deployment restoration steps, a post-recovery validation checklist, and recovery responsibilities | Whenever backup capability changes (e.g. a Pro-plan upgrade) or the recovery procedure itself is revised |
